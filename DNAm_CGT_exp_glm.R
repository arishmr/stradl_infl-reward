## Main regression analyses - associations between serum and DNAm CRP status and main affective cognition variables
## CGT-Win and CGT-Loss for serum CRP
...
colnames(CGT_mDNA)
data = data[,c(1:16)] #select columns in your dataset
...
targetdata_short = dat_sub[,c(1, # f.eid
                              ...
                              9:10, # dependent variables- BERT, CGT or FAGN summary measure
                              14,3:7)]    # covariates- age, sex, site, BMI 
no_fvar_long = NA
...
no_dvar_long = NA
no_dvar_short = 2  # how many dependent vars included in the short-format data e.g. 1 for BERT summary measure
vinteraction = T  #if you want to look at interaction effects change to T and the first covariate will be included as an interaction term
no_invar_long = 1
no_invar_short = NA
...
result.temp = UKB_lme_reg(targetdata_long,targetdata_short,
                          ...
                          vinteraction,1)
View(result.temp)
write.csv(result.temp, "results-DNAm_CGT_cigs_glm.csv") #save results as .csv file
