...
data = data[,c(1:25)] #select columns in your dataset
...
targetdata_short = dat_sub[,c(1, # f.eid
                              ...
                              4,15,5,7,6,9,8,14,13,11,10,12, # dependent variables- BERT, CGT or FAGN summary measure
                              19,16)]    # covariates- age, sex, site, BMI 
no_fvar_long = NA
...
result.temp = UKB_lme_reg(targetdata_long,targetdata_short,
                          ...
                          vinteraction,1)
View(result.temp)
write.csv(result.temp, "results-serum_fMRI_epsi_alcyest_glm.csv") #save results as .csv file
