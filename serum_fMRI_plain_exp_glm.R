## Main regression analyses - associations between serum and DNAm CRP status and key contrasts in brain regions

# Load packages
library(dplyr)

# Read in your dataset e.g. BERT data
data = fMRI_plain_serum
...
data = data[,c(1:13)] #select columns in your dataset
...
targetdata_short = dat_sub[,c(1, # f.eid
                              ...
                              4:7, # dependent variables- BERT, CGT or FAGN summary measure
                              10,8)]    # covariates- age, sex, site, BMI 
no_fvar_long = NA
...
no_dvar_long = NA
no_dvar_short = 4  # how many dependent vars included in the short-format data e.g. 1 for BERT summary measure
vinteraction = T  #if you wat to look at interaction effects change to T and the first covariate will be included as an interaction term
no_invar_long = 1
no_invar_short = NA
...
result.temp = UKB_lme_reg(targetdata_long,targetdata_short,
                          ...
                          vinteraction,1)
View(result.temp)
write.csv(result.temp, "results-serum_fMRI_plain_alcweek_glm.csv") #save results as .csv file
